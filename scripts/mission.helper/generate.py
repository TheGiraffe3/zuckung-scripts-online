...
def set_globals():
	...
	data_folders = [os.environ['REPOSITORY'].rstrip('\n')]
...
def write_mission(missions, mission_texts, mission_paths, blacklist, starts, data_folders): # write all to a single mission
	if os.path.isfile("multipluginname"):
		with open("multipluginname", 'r') as f:
			data_folder_string = f.read()
	else:
		data_folder_string = ''.join(data_folders)
	data_folder_capitalized = data_folder_string.title()
	with open('mission.helper.txt', 'w') as target:
		...
		target.writelines('\n')
		target.writelines('\n')
		target.writelines('mission "Mission Helper: ' + data_folder_string + '"\n')
		target.writelines('\tname "(Mission Helper: ' + data_folder_capitalized + ')"\n')
...
def run():
	...
	write_mission(missions, mission_texts, mission_paths, blacklist, starts, data_folders)
